...
import de.coaster.cringepvp.extensions.toOfflinePlayer
...
class ItemBuilder(material: Material, count: Int = 1, dsl: ItemBuilder.() -> Unit = {}) {
    ...
    fun asQuantity(quantity: Int): ItemBuilder {
        itemStack.amount = quantity
        return this
    }
    ...
    /**
     * Sets the custom model data for the item stack.
     *
    ...
    /**
     * Sets the custom model data for the item stack if the condition is true.
     *
     * @param modelData the custom model data to set
     * @param condition the condition to check
     * @return the updated ItemBuilder instance
     */
    fun customModelDataIf(modelData: Int, condition: Boolean = false): ItemBuilder {
        if (condition) {
...
/**
 * Converts a Material object to an ItemBuilder object.
 *
 * @param dsl A lambda function that customizes the ItemBuilder object.
 ...
 *
 * @param dsl A lambda function that can be used to configure the [ItemBuilder].
 * @return An instance of [ItemBuilder] with the specified configuration.
 */
